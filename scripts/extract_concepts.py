...
def openai_compatible_extract_mentions(client, model_name, batch_items, allowed_concepts):
    ...
    for attempt, use_format in enumerate([True, False]):
        ...
        # Strip thinking/reasoning blocks that DeepSeek may inject despite
        # being told not to. Handles both XML-style <thinking>...</thinking>
        # and markdown-fenced ```thinking...``` blocks.
        ...
        # Markdown-fenced: ```thinking ... ```  (with optional language tag)
        clean_content = re.sub(
            r'```thinking\b.*?```',
            '',
            clean_content,
        ...
        try:
            ...
            print(
                f"\n  [WARN] JSON parse failed after stripping thinking tags "
                f"(attempt with {'response_format' if use_format else 'no response_format'}): "
                f"{parse_err}",
                file=sys.stderr,
            )
            print(
                f"  [DEBUG] Raw content (first 500 chars): {content[:500]!r}",
                file=sys.stderr,
            )
            continue

    # All attempts exhausted -- the LLM is not returning parseable JSON.
    print(
        "\n  [WARN] openai_compatible_extract_mentions: all attempts "
        "exhausted, returning empty items list. Check model/provider compatibility.",
        file=sys.stderr,
    )
    return {"items": []}
