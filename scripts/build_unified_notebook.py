...
code(
    ...
    'USE_DECOMPOSITION = True    # sub-query split; REQUIRED by the competition (multi-hop / cross-document)\n'
    'USE_LLM_DECOMPOSITION = True   # drive split + complexity + facets with the (shared) Qwen; rule fallback kept\n'
    'USE_GENERATOR     = True    # IRAC answer generation (one 4-bit Qwen)\n'
    '# --- Combined recall layer (teammate ideas, de-hardcoded) -------------\n'
    'USE_MULTI_VARIANT = True    # retrieve over original+atomic+facet variants, rerank ONCE (recall ↑, latency flat)\n'
    'USE_COMPLEXITY_K  = True    # let plan complexity set per-query K (simple≈1, complex up to 10)\n'
    'COVERAGE_QUOTA    = True    # admit >=1 article per sub-query angle (de-hardcoded role-completion)\n'
    'MAX_VARIANTS      = 5       # hard cap on dense searches/question (cross-encoder still runs once)\n'
...
code(
    ...
    '    use_multi_variant=USE_MULTI_VARIANT, use_complexity_k=USE_COMPLEXITY_K,\n'
    '    coverage_quota=COVERAGE_QUOTA, max_variants=MAX_VARIANTS,\n'
    ')\n'
    'print("[pipeline] built | dense=%s rerank=%s decompose=%s llm_decompose=%s generator=%s"\n'
    '      % (USE_DENSE, USE_RERANK, USE_DECOMPOSITION, USE_LLM_DECOMPOSITION, USE_GENERATOR))\n'
    'print("[recall-layer] multi_variant=%s complexity_k=%s coverage_quota=%s max_variants=%d"\n'
    '      % (USE_MULTI_VARIANT, USE_COMPLEXITY_K, COVERAGE_QUOTA, MAX_VARIANTS))\n'
)
...
code(
    ...
    '    _timings.append(_t)\n'
    '    print("id=%s cx=%s n_sub=%d n_var=%d mv=%s | decompose=%.2fs retrieve=%.2fs '
    'select=%.2fs generate=%.2fs total=%.2fs"\n'
    '          % (_qid, _t["complexity"], _t["n_sub"], _t["n_variants"], _t["multi_variant"],\n'
    '             _t["decompose"], _t["retrieve"], _t["select"], _t["generate"], _t["total"]))\n'
    '\n'
    ...
    'print("  multi-variant   : %.0f%% (%d/%d)  mean variants: %.2f  max variants: %d"\n'
    '      % (100 * _summary["multi_variant_rate"], _summary["multi_variant_count"],\n'
    '         _summary["n"], _summary["mean_n_variants"], _summary["max_n_variants"]))\n'
    ...
    '# Single-rerank invariant: retrieve time should NOT scale with variant count.\n'
    '# If mean variants > 1 but retrieve_mean stayed near the single-query cost,\n'
    '# the gather-then-rerank-ONCE design is holding.\n'
    'if _summary["mean_n_variants"] > 1.0:\n'
    '    print("  [invariant] mean %.1f variants/query but ONE rerank pass each "\n'
    '          "(retrieve=%.2fs) -> multi-variant recall at ~flat GPU cost"\n'
    '          % (_summary["mean_n_variants"], _summary["retrieve_mean"]))\n'
    'if _summary["total_mean"] > 120:\n'
    '    print("  [WARN] mean/query %.1fs exceeds the 1-2 min budget — lower '
    'MAX_VARIANTS or RERANK_POOL" % _summary["total_mean"])\n'
)

md(
    "## 4c. Decomposition debug — raw Qwen output vs parsed sub-queries\n"
    "\n"
    "Calls the router directly on a few questions and prints the **raw LLM text**\n"
    "next to the **parsed sub-queries**. Use this to confirm Qwen emits parseable\n"
    "JSON on GPU. `source=\"llm\"` means the JSON parsed; `source=\"rule_fallback\"`\n"
    "means the LLM call/parse failed and the deterministic split was used instead.\n"
    "Only meaningful when `USE_DECOMPOSITION and USE_LLM_DECOMPOSITION` are True\n"
    "(otherwise `pipe.router` is None or rule-based)."
)

code(
    'import json\n'
    '\n'
    'if pipe.router is None:\n'
    '    print("[decomp-debug] no router (USE_DECOMPOSITION is False) — skipping")\n'
    'else:\n'
    '    _dbg_q = Path(INPUT_QUERIES) if str(INPUT_QUERIES).strip() else Path(f"{DEV_DIR}/questions.json")\n'
    '    _dbg_qs = json.loads(_dbg_q.read_text(encoding="utf-8"))[:3]\n'
    '    _n_llm, _n_fb = 0, 0\n'
    '    for _r in _dbg_qs:\n'
    '        _q = _r.get("question") or _r.get("query") or ""\n'
    '        _d = pipe.router.route(_q)\n'
    '        _n_llm += int(_d.source == "llm")\n'
    '        _n_fb  += int(_d.source != "llm")\n'
    '        print("=" * 72)\n'
    '        print("Q:", _q)\n'
    '        print("source        :", _d.source)\n'
    '        print("should_decompose:", _d.should_decompose, "| n_sub:", _d.num_sub_queries)\n'
    '        print("parsed sub_queries:")\n'
    '        for _i, _sq in enumerate(_d.sub_queries):\n'
    '            print("   [%d] %s" % (_i, _sq))\n'
    '        _raw = getattr(_d, "raw_llm_output", None)\n'
    '        print("raw_llm_output:")\n'
    '        print("   " + (repr(_raw)[:600] if _raw else "<None — fallback path>"))\n'
    '    print("=" * 72)\n'
    '    print("[decomp-debug] llm-parsed=%d  fallback=%d  (of %d)"\n'
    '          % (_n_llm, _n_fb, len(_dbg_qs)))\n'
    '    if _n_fb and _n_llm == 0:\n'
    '        print("  WARNING: every call fell back to rule-based — Qwen is not "\n'
    '              "emitting parseable JSON. LLM decomposition adds cost with no benefit.")\n'
)
